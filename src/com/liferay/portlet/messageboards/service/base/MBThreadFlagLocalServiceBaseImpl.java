...
public abstract class MBThreadFlagLocalServiceBaseImpl
	implements MBThreadFlagLocalService, IdentifiableBean {
	/*
	 * NOTE FOR DEVELOPERS:
	 *
	...
	public MBThreadFlag addMBThreadFlag(MBThreadFlag mbThreadFlag)
		...
		mbThreadFlag = mbThreadFlagPersistence.update(mbThreadFlag, false);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.reindex(mbThreadFlag);
			}
	...
	public void deleteMBThreadFlag(long threadFlagId)
		...
		MBThreadFlag mbThreadFlag = mbThreadFlagPersistence.remove(threadFlagId);

		Indexer indexer = IndexerRegistryUtil.getIndexer(getModelClassName());

		if (indexer != null) {
			try {
				indexer.delete(mbThreadFlag);
			}
			catch (SearchException se) {
				if (_log.isWarnEnabled()) {
					_log.warn(se, se);
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public void deleteMBThreadFlag(MBThreadFlag mbThreadFlag)
		throws SystemException {
	...
	public MBThreadFlag updateMBThreadFlag(MBThreadFlag mbThreadFlag,
		...
		mbThreadFlag = mbThreadFlagPersistence.update(mbThreadFlag, merge);
	...
	private static Log _log = LogFactoryUtil.getLog(MBThreadFlagLocalServiceBaseImpl.class);
